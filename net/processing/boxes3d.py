...
def box3d_to_rgb_box_obj(boxes3d, calib, R0_rect, Tr_velo_to_cam):
    if (cfg.DATA_SETS_TYPE == 'kitti'):
        ...
        #     if np.sum(projection + np.abs(projection)) == np.sum(2*np.abs(projection)):
        #         projection_rm = projection
        #         projections_rm.append(projection_rm)
        # # print (np.array(projections_rm))
